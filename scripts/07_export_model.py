...
def find_training_config(dataset_dir: Path) -> Optional[Path]:
    ...
    if config_path.exists():
        # Use the centralized logger
        logger.info(f"✅ Found training config at: {config_path}")
        return config_path
    # Use the centralized logger
    logger.warning(f"⚠️ Could not find training config.json in {dataset_dir / 'cache'}")
    return None

def main():
    """Main function to export the trained Piper model to ONNX format."""
    # Use the centralized logger
    logger.info("🚀 Starting Stage 7: Exporting Model to ONNX...")
    ...
    try:
        ...
        if not latest_ckpt:
            # Use the centralized logger
            logger.error("❌ No trained checkpoint found to export. Please run stage 7 first.")
            sys.exit(1)
        ...
        # Use the centralized logger
        logger.info("🔥 Exporting model... This may take a moment.")
        # Use the centralized logger
        ...
        if result.returncode == 0:
            ...
            # Use the centralized logger
            logger.error(f"❌ Model export failed with exit code {result.returncode}.")
            # Use the centralized logger
        ...
        if training_config_path:
            ...
            # Use the centralized logger
            logger.info(f"✅ Copied training config to {config_path}")
        else:
            # Use the centralized logger
            logger.warning("⚠️ Could not find the training config.json. The exported model may not work without it.")

        # Use the centralized logger
        logger.info("\n🎉 Model export complete! Your voice is ready for inference.")

    except subprocess.CalledProcessError as e:
        # Use the centralized logger
        logger.error(f"❌ A subprocess error occurred during export: {e}")
        # Use the centralized logger
